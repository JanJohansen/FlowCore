			...
			<div class="property-editor-modal" @click.stop>
				...
				<div class="modal-content">
					<div v-for="(input, key) in nodeInputs" :key="key" class="input-group">
						...
						<!-- Object/Array input -->
						<div v-else-if="input.valueType === 'object' || input.valueType === 'array'" class="json-editor">
							<textarea
								:id="key"
								v-model="localInputValues[key]"
								rows="3"
...
const nodeInputs = computed(() => {
	const inputs = props.nodeDefinition?.ins || {}
...
watch(
	...
	(newNode) => {
		...
		Object.keys(nodeInputs.value).forEach((key) => {
			const value = newNode.config?.ins?.[key]?.value
...
function handleSave() {
	...
	Object.entries(nodeInputs.value).forEach(([key, input]) => {
		...
		switch (input.valueType) {
			...
			case 'array':
				try {
					processedValues[key] = typeof rawValue === 'string' ? 
			...
			case 'enum':
				...
				if (input.enum.includes(rawValue)) {
