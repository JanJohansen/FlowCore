...
describe('CoreDB New API', () => {
    ...
    describe('deprecated on() method', () => {
        test('on() should show deprecation warning and work like onPatch()', () => {
            const consoleSpy = jest.spyOn(console, 'warn').mockImplementation()
            const callback = jest.fn()
            
            db.on('user1', callback)
            
            expect(consoleSpy).toHaveBeenCalledWith('CoreDB.on() is deprecated. Use onPatch() instead.')
            
            // Should work like onPatch
            db.patch('user1', { name: 'John' })
